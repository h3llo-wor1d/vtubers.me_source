...
if ($action == "login") {
    ...
    if ($cl['config']['user_signup'] == "on" || (not_empty($invite_link) && cl_verify_invite_code($invite_code))) {
        ...
        //TODO: Rename recaptcha variables to hcaptcha
        if ($cl["config"]["google_recaptcha"] == "on") {
            $data['err_code'] = "hcaptcha_error";
            $recaptcha_code   = fetch_or_get($_POST['h-captcha-response'], false);
            $recaptcha_verif  = file_get_contents(cl_strf("https://hcaptcha.com/siteverify?secret=%s&response=%s", $cl["config"]["google_recap_key2"], $recaptcha_code));

            if (not_empty($recaptcha_verif)) {
                $recaptcha_verif = json($recaptcha_verif);

                if (is_array($recaptcha_verif) && not_empty($recaptcha_verif["success"]) && $recaptcha_verif["success"]) {
                    $data['err_code'] = false;
                } else {

                    $data['err_code'] = $recaptcha_verif["error-codes"];
                }
            }
        }
        ...
        if (empty($data['err_code'])) {
            if ($cl['config']['acc_validation'] == 'off') {
                ...
                $insert_data      = array(
                    ...
                    'display_settings' => json(array("color_scheme" => $cl["config"]["default_color_scheme"], "font_type" => $cl["config"]["default_font_type"], "background" => $cl["config"]["default_bg_color"]), true)
                );
