...
if ($action == 'upload_post_image') {
    ...
    $bg_color   = fetch_or_get($_POST["bg"], "white");
    $skin_color = fetch_or_get($_POST["color"], "blue");
    $font_type = fetch_or_get($_POST["font_type"], "inter");

    if (in_array($bg_color, array_keys($cl["bg_colors"])) && in_array($font_type, array_keys($cl["font_types"])) && in_array($skin_color, array_keys($cl["color_schemes"]))) {
        $data['status'] = 200;

        cl_update_user_data($me["id"], array(
            "display_settings" => json(array(
                "color_scheme" => cl_text_secure($skin_color),
                "font_type" => cl_text_secure($font_type),
