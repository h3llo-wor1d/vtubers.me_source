...
function cl_get_media($media = '', $is_upload = false)
{
    ...
    if ($config['as3_storage'] == 'on') {
        ...
        $media_url  = cl_strf("https://%s.nyc3.digitaloceanspaces.com/%s", $as3_bucket, $media);
        return $media_url;
    } else {
...
function cl_get_media_placeholder($media = "")
{
    ...
    if ($config['as3_storage'] == "on") {
        $as3_bucket = $config['as3_bucket_name'];
        $media_url  = cl_strf("https://%s.nyc3.digitaloceanspaces.com/upload/default/%s.png", $as3_bucket, $media);
        return $media_url;
    } else {
...
function cl_cropimg($max_width, $max_height, $source_file, $dst_dir, $quality = 80)
{
    ...
    if ($width_new > $width) {
        ...
        @imagecopyresampled($dst_img, $src_img, 0, 0, 0, intval($h_point), intval($max_width), intval($max_height), intval($width), intval($height_new));
    } else {
        $w_point = (($width - $width_new) / 2);
        @imagecopyresampled($dst_img, $src_img, 0, 0, intval($w_point), 0, intval($max_width), intval($max_height), intval($width_new), intval($height));
    }
...
function cl_upload($data = array())
{
    ...
    if (move_uploaded_file($data['file'], $filename)) {
        if (in_array($file_ext, array('gif', 'png', 'jpeg', 'jpg', 'webp')) == true) {
            ...
            // TODO: Cropping webp isn't supported yet.
            if ($file_ext === 'webp') {
                $result['cropped'] = $filename;
            } else if (not_empty($data['crop'])) {
    ...
    if ($cl['config']['as3_storage'] == 'off') {
        ...
        if (empty($cl['config']['as3_api_key'])) {
            ...
            try {
                ...
                if ($del_localfile == "Y") {
                    if ($amazon_s3->doesObjectExist('cdn.vtubers', $filename)) {
